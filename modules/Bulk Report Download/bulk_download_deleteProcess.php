...
if ($_SERVER['REQUEST_METHOD'] !== 'POST') {
    $URL .= '&return=error2'; // Invalid request method
    header("Location: {$URL}");
    exit;
}
...
// Validate the log ID
if (empty($logID)) {
    $URL .= '&return=error3'; // Missing or invalid required variable
    header("Location: {$URL}");
    exit;
}

// Proceed to delete the log entry
try {
    $query = "DELETE FROM bulk_download_logs WHERE logID = :logID";
    ...
    if ($stmt->rowCount() > 0) {
        ...
        // Log ID not found or already deleted
        $URL .= '&return=error4';
    }
} catch (Exception $e) {
