            ...
            if ($result->rowCount() < 1) {
                ...
                while ($row = $result->fetch()) {
                    ...
                    if ($isAdmin) {
                        ...
                        $columnNameEncoded = urlencode($row['name']);
                        if ($isLocked) {
                            echo "<img title='".__('Locked')."' src='./themes/".$session->get('gibbonThemeName')."/img/key.png' style='opacity:0.4;'/> ";
                            echo "<a href='".$session->get('absoluteURL').'/modules/'.$session->get('module')."/internalAssessment_manage_lockProcess.php?gibbonCourseClassID=$gibbonCourseClassID&gibbonInternalAssessmentColumnID=".$row['gibbonInternalAssessmentColumnID']."&action=unlock&address=".$session->get('address')."'><img title='".__('Unlock this class only')."' src='./themes/".$session->get('gibbonThemeName')."/img/iconTick.png'/></a> ";
                            echo "<a href='".$session->get('absoluteURL').'/modules/'.$session->get('module')."/internalAssessment_manage_lockProcess.php?gibbonCourseClassID=$gibbonCourseClassID&gibbonInternalAssessmentColumnID=".$row['gibbonInternalAssessmentColumnID']."&action=unlockAll&columnName={$columnNameEncoded}&address=".$session->get('address')."'><img title='".__('Unlock All Classes - make visible to teachers in all classes')."' src='./themes/".$session->get('gibbonThemeName')."/img/iconTick_double.png'/></a> ";
                        } else {
                            echo "<a href='".$session->get('absoluteURL').'/modules/'.$session->get('module')."/internalAssessment_manage_lockProcess.php?gibbonCourseClassID=$gibbonCourseClassID&gibbonInternalAssessmentColumnID=".$row['gibbonInternalAssessmentColumnID']."&action=lock&address=".$session->get('address')."'><img title='".__('Lock this class only')."' src='./themes/".$session->get('gibbonThemeName')."/img/key.png'/></a> ";
                            echo "<a href='".$session->get('absoluteURL').'/modules/'.$session->get('module')."/internalAssessment_manage_lockProcess.php?gibbonCourseClassID=$gibbonCourseClassID&gibbonInternalAssessmentColumnID=".$row['gibbonInternalAssessmentColumnID']."&action=lockAll&columnName={$columnNameEncoded}&address=".$session->get('address')."'><img title='".__('Lock All Classes - hide from teachers in all classes')."' src='./themes/".$session->get('gibbonThemeName')."/img/page_new_multi.png'/></a> ";
                        }
                    }
                    if (!$isLocked) {
